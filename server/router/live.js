...
router.post('/getLiveList', (req, res) => {
    ...
    let sql = `SELECT z.* FROM (
                    SELECT l.*, t.name, GROUP_CONCAT(DISTINCT tm.model, '_', tm.platform, '_', IF(tm.shop_type IS NULL, '', tm.shop_type), '_', IF(tm.shop_name IS NULL, '', tm.shop_name), '_', IF(tm.account_name IS NULL, IF(tm.group_name IS NULL, tm.provide_name, tm.group_name), account_name)) as models,
